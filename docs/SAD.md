...
## 0.3 简洁编辑与可选问答助手（2026-09-24）

用户已要求兼容 OpenAI 的 API 接入。普通模式只显示权重、评分和备注；专业模式展示原有锚点、摆幅与证据元数据。`advancedUiExpanded` 只控制展示，不参与计算。组件 `SimpleComparison` 与专业视图共享同一 Decision，切换不会清空数据。

问答链路为 `DecisionAssistant → assistantClient → 用户配置的 Chat Completions 服务`；纯领域模块 `assistant.ts` 负责最小上下文投影、DTO 校验和草稿转换。API 密钥与对话仅在内存，配置及自定义提示词写入独立 localStorage。手动发送前显示传输范围和地址；服务需支持 CORS，不增加后端或开放代理。

助手只提出候选设计；通过 schema 后展示预览，用户明确应用才进入 `DecisionSession.update`。比较内容指纹防止过期建议覆盖中途修改；应用会重建结构、清空评分、清理验证任务的失效绑定，保留执行历史与快照。详情与验收见 [问答助手设计](./assistant-design.md)。下文 0.2/0.1 的“无外部通信”仅适用于未启用助手的本地流程。

> 2026-09-24 实现状态：0.2 已交付比较、验证与投入、行动与复盘三个工作区。证据、投入、行动、预测和复盘内嵌于可选 `Decision.workflow`，不引入新 object store；只有确定性加权模型参与排名。下述高级数值模型仍是后续目标。

> 2026-09-21 增量设计：本轮以 [实施计划](./implementation-plan.md) 为新的功能与责任划分基线。新增比较组件与独立工作流领域模块；证据、投入、行动和复盘内嵌于 Decision，不引入服务或新数据库。下述早期高级数值模型仍为目标设计，本轮只有基础加权模型参与排名。
...
本地比较和存储不与外部服务通信；0.3 的可选问答助手仅在用户发送时访问配置的模型 API。导出只在用户明确操作时创建本地文件；导入文件在验证通过前不得修改现有数据。
